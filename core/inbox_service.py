...
from common.thread_renderer import render_history
...
class InboxService:
    """Correspondence logic. Talks only to ports and domain models."""
    ...
    def send(self, draft: Draft) -> UnreadMessage:
        ...
        if body_html and draft.include_history and history:
            body_html += render_history(history)
    ...
    def poll(
        self, mailbox: EmailAddress, thread: ThreadSlug | None = None, limit: int = 50
    ) -> list[UnreadMessage]:
        return self._repository.list_unread(recipient=str(mailbox), thread=thread, limit=limit)
    ...
    def drain(self, purge: bool = False, limit: int = 200) -> list[UnreadMessage]:
        ...
        for captured in self._mailbox.capture(limit=limit):
            ...
            if self._repository.exists_by_rfc_id(captured.rfc_message_id):
                continue
            claimed.append(self._repository.add(_from_capture(captured)))
        if purge and taken:
...
def _from_capture(captured: CapturedMessage) -> UnreadMessage:
    ...
    return UnreadMessage(
        content=Correspondence(
            ...
            sender=EmailAddress(captured.sender),
            recipient=EmailAddress(captured.recipient),
            author=author,
